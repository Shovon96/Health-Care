...
import { createDoctorZodSchema, updateDoctorZodSchema } from "@/src/zodValidators/doctor.validation";
...
export async function updateDoctorById(id: string, _prevState: any, formData: FormData) {
    const experienceValue = formData.get("experience");
    const appointmentFeeValue = formData.get("appointmentFee");
    ...
    const validatedPayload = zodValidation(validationPayload, updateDoctorZodSchema);
