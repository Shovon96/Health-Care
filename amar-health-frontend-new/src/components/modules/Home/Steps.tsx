...
const StepCard = ({ icon: Icon, title, description, index }: { icon: React.ElementType, title: string, description: string, index: number }) => {
    ...
    return (
        <Card className={`group relative overflow-hidden glass-card-strong hover-lift hover-glow transition-all duration-500 hover:-translate-y-2 backdrop-blur-sm border-white/20`}>
            ...
            <div className={`absolute inset-0 bg-linear-to-br ${gradients[index % 8]} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
            <div className="absolute -top-4 -right-4 w-24 h-24 bg-linear-to-br from-white/10 to-transparent rounded-full blur-xl group-hover:scale-150 transition-transform duration-700 animate-morph" />
            <div className="absolute -bottom-4 -left-4 w-20 h-20 bg-linear-to-tr from-white/5 to-transparent rounded-full blur-lg group-hover:scale-125 transition-transform duration-500 animate-bounce-gentle" />

            <CardContent className="relative p-6 z-10">
                <div className="flex flex-col items-center text-center space-y-4">
                    {/* Enhanced icon with advanced gradient background and multiple glow effects */}
                    <div className={`relative p-4 rounded-2xl bg-linear-to-br ${iconBgGradients[index % 8]} shadow-2xl group-hover:shadow-3xl group-hover:scale-110 transition-all duration-300 animate-gradient`}>
            ...
            <div className="absolute inset-0 rounded-lg bg-linear-to-br from-white/20 via-transparent to-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
            <div
...
const Steps = () => {
    return (
        <section className="py-24 relative overflow-hidden">
            ...
            <div className="absolute inset-0 bg-linear-to-br from-indigo-900 via-purple-900 to-pink-900 animate-gradient" />
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="text-center mt-16 animate-fade-in-up" style={{ animationDelay: '1400ms' }}>
                    <div className="inline-flex items-center gap-2 text-sm text-white/60 glass-card rounded-full px-6 py-2">
                        ...
                        <div className="w-12 h-px bg-linear-to-l from-transparent via-white/30 to-transparent" />
                    </div>
                </div>
            </div>
        </section>
